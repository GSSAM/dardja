...
app.post('/api/correct-bac-subject', async (req, res) => {
    ...
    try {
        ...
        if (fileId) {
            // إضافة &confirm=t لتخطي صفحة التحذير من الفيروسات للملفات الكبيرة في Google Drive
            ...
            try {
                ...
                if (pdfResponse.ok) {
                    // التحقق من نوع المحتوى لضمان عدم استلام صفحة HTML (مثل صفحة تسجيل الدخول أو التحذير)
                    const contentType = pdfResponse.headers.get('content-type');
                    if (contentType && contentType.includes('text/html')) {
                        return res.status(502).json({ error: "فشل استخراج الـ PDF: خوادم Google Drive أعادت صفحة ويب بدلاً من الملف. قد يكون الملف خاصاً أو محذوفاً." });
                    }
                    ...
                    // تحقق إضافي من الحجم الحقيقي للـ Buffer المحمل
